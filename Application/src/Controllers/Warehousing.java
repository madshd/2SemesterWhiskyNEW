 ...
 * The method iterates through all warehouses and their storage racks, checking if any of the storage racks contain null items.
 * If a storage rack contains null items, it indicates that the storage rack has available space.
 * @return a map containing warehouses and their respective storage racks that have available space.
 */
    ...
    public static Ingredient createIngredientAndAdd(String name, String description, int batchNo, LocalDate productionDate, LocalDate expirationDate,
        ...
        Ingredient ingredient = new Ingredient(name, description, batchNo, productionDate, expirationDate, quantity, supplier, unit, ingredientType);
        ...
        try {
            ...
            throw new IllegalArgumentException("Failed to add ingredient to the specified storage rack: " + e.getMessage());
        }
    ...
    /**
     * Moves an item from one warehouse to another, updating the storage racks and notifying observers.
     *
    ...
    /**
     * Moves an item from one storage rack to another within the same or different warehouses, updating the storage racks and notifying observers.
     *
     * @param item the item to be moved
     * @param fromStorageRack the storage rack from which the item is being moved
     * @param fromIndex the index of the item in the source storage rack
    ...
    public static void moveItemBetweenStorageRacks(Item item, StorageRack fromStorageRack, int fromIndex, StorageRack toStorageRack, int toIndex) {
        if (fromStorageRack.getItemLocation(item) != fromIndex) {
            throw new IllegalArgumentException("Item not found at the specified index in the source storage rack.");
        }
